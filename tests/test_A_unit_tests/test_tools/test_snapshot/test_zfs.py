...
def test_zfs_snapshot():
    ...
    with Environment(
        ...
        assert log[6].argv[1:] == ["list", "-t", "snapshot", "-H", "-o", "name,creation", "-p", "-d", "1", "tank/services/immich"]
        ...
        # Let's test calling generic exception if subprocess fails (which we can force by passing a path that mock handled as error)
